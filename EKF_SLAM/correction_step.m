...
for i = 1:m
	...
	if(observedLandmarks(landmarkId)==false)
        ...
        mu(2*z(i).id+2) = mu(1) + z(i).range * cos(z(i).bearing + mu(3));
        mu(2*z(i).id+3) = mu(2) + z(i).range * sin(z(i).bearing + mu(3));
	...
	mx = mu(2*landmarkId+2);
    my = mu(2*landmarkId+3);
    delta = [mx - mu(1); my - mu(2)];
    q = delta' * delta;
